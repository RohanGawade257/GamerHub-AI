...
function linkClassName({ isActive }) {
  return [
    "rounded-xl border px-3 py-1.5 text-sm font-semibold tracking-wide transition-all duration-300 lg:px-3 lg:py-1 lg:text-sm",
    isActive
...
function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-40 border-b border-white/40 bg-white/70 shadow-xl shadow-indigo-100/40 backdrop-blur-xl dark:border-zinc-700/70 dark:bg-zinc-950/65 dark:shadow-cyan-950/30">
      <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3 px-4 py-3 lg:flex-nowrap lg:items-start lg:gap-2.5 lg:px-4 lg:py-1.5">
        <Link to={token ? "/dashboard" : "/login"} className="group inline-flex items-center gap-2 lg:self-start lg:gap-2">
          <span className="h-2.5 w-2.5 rounded-full bg-cyan-400 shadow-[0_0_12px_rgba(34,211,238,.9)] transition-all duration-300 group-hover:scale-125 lg:h-2.5 lg:w-2.5" />
          <span className="bg-gradient-to-r from-indigo-600 via-blue-500 to-cyan-500 bg-clip-text text-lg font-extrabold tracking-wide text-transparent dark:from-indigo-300 dark:via-cyan-300 dark:to-blue-300 lg:text-lg">
            Gamer-Hub
          </span>
        </Link>
        ...
        <div className="hidden flex-wrap items-center gap-2 md:flex lg:ml-auto lg:self-start lg:gap-2.5">
          {token ? (
            <>
          ...
          {token && (
            <button
              ...
              className="rounded-xl bg-gradient-to-r from-indigo-600 to-blue-600 px-4 py-1.5 text-sm font-semibold text-white shadow-lg shadow-indigo-500/25 transition-all duration-300 hover:-translate-y-0.5 hover:scale-105 hover:shadow-xl hover:shadow-indigo-500/35 lg:px-4 lg:py-1.5 lg:text-sm"
            >
              Logout
            </button>
          )}
          ...
          {token ? (
            <div className="ml-2 flex items-center gap-3 rounded-xl border border-white/60 bg-white px-4 py-1.5 shadow-md dark:border-zinc-700 dark:bg-zinc-800 lg:ml-2 lg:gap-3 lg:px-4 lg:py-1.5">
              <div className="relative">
                <div className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-full bg-indigo-100 text-sm font-bold text-indigo-700 dark:bg-indigo-900/60 dark:text-indigo-200 lg:h-8 lg:w-8 lg:text-sm">
                  {profileImage ? (
              ...
              <span className="text-lg font-semibold text-slate-800 dark:text-zinc-100 lg:text-base">{displayName}</span>
            </div>
          ) : null}
        </div>
      </div>
